  ...
  return (
    <div style={S.content}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <h2 style={{ ...S.sectionTitle, margin: 0 }}>最新のお知らせ</h2>
        <button
          onClick={() => { window.location.href = "https://www.youtube.com/@ManilaHapons"; }}
          style={{ display: "flex", alignItems: "center", gap: 6, background: "#FF0000", border: "none", borderRadius: 20, padding: "7px 14px", color: "#fff", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>
          ▶ YouTube
        </button>
      </div>
      {loading && <Loading />}
...
// iOS Safari/Chrome では dangerouslySetInnerHTML 等で動的に挿入した
// <a target="_blank"> が単純タップで反応しない（長押し→新しいタブでは開ける）
// という既知の癖があるため、target="_blank" には頼らず、タップ時にJSで
// 同一タブ内へ明示的に遷移させる。
...
const autoLink = (html) => {
  ...
  return cleaned
    ...
    .map((part) => {
      if (/^<a\b/i.test(part)) {
        // 既存リンク：色を補って必ずリンクらしく見せる（targetは付けない＝同一タブ遷移）
        let a = part.replace(/\s+target\s*=\s*(["'])_blank\1/i, "");
        if (!/\bstyle\s*=/i.test(a)) a = a.replace(/^<a\b/i, '<a style="color:#1E88E5;word-break:break-all;"');
        return a;
      }
      return part.replace(
        /(https?:\/\/[^\s<>"]+)/g,
        '<a href="$1" style="color:#1E88E5;word-break:break-all;">$1</a>'
      );
    })
    .join("");
};
